...
.pill.ok { background: #06331f; } .pill.bad { background: #3a1414; } .pill.pending { background: #33280a; color: #f0b429; }
</style>
</head><body>
<header>
  <h1>OAuth Refresh &mdash; self-service login</h1>
  <p class="warn">Sensitive: this UI mints provider tokens and writes them to OpenBao. Keep it LAN-only and behind SSO.</p>
</header>
<main>{{.Body}}</main>
</body></html>{{end}}
...
<div class="card"><div class="row">
  <div><span class="name">{{.Name}}</span><span class="kind">{{.Kind}} flow</span></div>
  <div class="status">
    {{if .Err}}<span class="bad">read error</span>
    {{else if not .Seeded}}<span class="muted">not logged in</span>
    {{else if .TokenValid}}<span class="ok">&#9679; valid</span> <span class="muted">until {{.Expiry.UTC.Format "2006-01-02 15:04 UTC"}}</span>
    {{else}}<span class="bad">&#9679; expired</span>{{end}}
  </div>
</div>
<div class="row" style="margin-top:12px">
  <span class="muted">{{if .Seeded}}Re-login to replace the stored credential.{{else}}No credential seeded yet.{{end}}</span>
  <form method="post" action="/login/{{.Name}}"><button class="btn" type="submit">Log in with {{.Name}}</button></form>
</div></div>
{{else}}
...
<div class="card">
  <div class="name">{{.Provider}} <span class="kind">device login</span></div>
  ...
  {{else if eq (printf "%s" .State) "pending"}}
    ...
    <p class="muted">Waiting for you to approve&hellip; this page refreshes automatically.</p>
  {{else}}
...
<div class="card">
  <div class="name">{{.Provider}} <span class="kind">browser login</span></div>
  {{if eq (printf "%s" .State) "authorized"}}
    <p class="ok">&#10004; Logged in. The credential was written to OpenBao and will be kept fresh automatically.</p>
    <a class="btn" href="/">Back to dashboard</a>
  {{else}}
